<script lang="ts">
    ...
    // Efecto reactivo: se ejecuta al entrar a la sala o cuando cambia data
    // Abre un nuevo WebSocket, marca las notificaciones como leídas y carga los mensajes
    $effect(() => {
        if (!browser) return;
        mensajes = data.mensajes ?? [];

        // Marcar notificaciones de esta sala como leídas en el servidor
        ...
        // Cierra el WebSocket anterior antes de abrir uno nuevo (cambio de sala)
        if (ws) {
            ws.close();
        }
        ...
        return () => ws.close(); // Cleanup al cambiar de sala o desmontar el componente
    });
